...
const Community = () => {
  ...
  const fetchUserCreations = async()=>{
    // fetch creations from backend using user id
    try {
  ...
  const imageLikeToggle = async (id) => {
      ...
      if(data.success){
        ...
        await fetchUserCreations(); //refresh the liked fresh creations data
      }else{
        toast.error(data.message);
      }
      } catch (error) {
  ...
  useEffect(()=>{
    if(user){
      fetchUserCreations() //if user is logged in then only fetch creations
    }
  }, [user])

  return !loading ? ( //when loading is false
    <div className='flex-1 h-full flex flex-col gap-4 p-6'>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Palette className="w-7 h-7 text-[#8E37EB]" />
